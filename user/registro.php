...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($nivel_educativo)) {
        $mensaje = "Debes seleccionar un nivel educativo.";
    } else {
        // Verificar si la matrícula o credencial ya está registrada
        $verificar = "SELECT * FROM usuarios WHERE matricula_credencial = '$matricula_credencial'";
        $resultado = $conn->query($verificar);

        if ($resultado->num_rows > 0) {
            $mensaje = "La matrícula o credencial ya está registrada. Intenta con otra.";
        } else {
